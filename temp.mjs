        ...
        let isDesktopMode = localStorage.getItem('desktopMode') !== null
            ...
            : true; // Desktop mode ON by default; user can switch to mobile via the toggle
        ...
        onAuthStateChanged(auth, async (rawUser) => {
            if (rawUser) {
                // Determine Real Email (mapping code-based user email back to real one)
                let realEmail = rawUser.email || '';
                if (realEmail.toLowerCase().includes('+code@')) {
                    realEmail = realEmail.replace('+code@', '@');
                }
                const uEmail = realEmail.toLowerCase();
                const MAIN_ADMINS = ['vono.niccolo@gmail.com'];
                userRole = 'user';
                supervisorPermissions = {};
                isAdmin = false;

                const proxyUser = new Proxy(rawUser, {
                    get(target, prop) {
                        if (prop === 'email') return realEmail;
                        if (prop === 'displayName') {
                            if (target.displayName && target.displayName !== rawUser.email) return target.displayName;
                            return realEmail.split('@')[0].split('.').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' ');
                        }
                ...
                if (userRole === 'user') {
                    ...
                    try {
                        ...
                        const permSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'permissions'));
                        let userPerms = { instruments: true, inventory: false }; // Strict Default
                        if (permSnap.exists() && permSnap.data().users && permSnap.data().users[uEmail]) {
                            userPerms = permSnap.data().users[uEmail];
                        }
                        window.currentUserPermissions = userPerms;
                        
                        if (!userPerms.inventory) {
                            document.getElementById('btn-home-inventory').classList.add('hidden');
                        }
                        if (!userPerms.instruments) {
                            document.getElementById('btn-home-bookings').classList.add('hidden');
                        }
                        if (!userPerms.instruments && !userPerms.inventory) {
                            document.getElementById('btn-home-scanner').classList.add('hidden');
                        }
                    ...
                    // Admins and supervisors have full module access default
                    window.currentUserPermissions = { instruments: true, inventory: true };
                }
                ...
                const displayName = proxyUser.displayName || proxyUser.email || proxyUser.uid.slice(0, 8);
                ...
                // Show Statistiche button for admins/supervisors
                const statBtn = document.getElementById('btn-home-statistiche');
                if (statBtn) {
                    statBtn.classList.toggle('hidden', userRole === 'user');
                }

                // Start periodic booking archival
                archiveExpiredBookings();
                if (!window._archiveInterval) {
                    window._archiveInterval = setInterval(archiveExpiredBookings, 5 * 60 * 1000);
                }
                logAction('LOGIN', 'AUTH', `User logged in: ${user.email} Role: ${userRole}`);
                window.resetInactivityTimer(); // start inactivity timer on login
            } else {
        ...
        window.logoutUser = async (isAuto = false) => {
            if (isAuto || confirm('Sei sicuro di voler uscire?')) {
                clearTimeout(window._inactivityWarningTimer);
                clearTimeout(window._inactivityLogoutTimer);
                clearInterval(window._inactivityCountdownInterval);
                document.getElementById('inactivity-toast')?.classList.add('hidden');
                if (currentUser) logAction('LOGOUT', 'AUTH', `User logged out${isAuto ? ' (auto-inactivity)' : ''}: ${currentUser.email}`);
                adminMode = false;
                isAdmin = false;
                await signOut(auth);
            }
        ...
        // --- INACTIVITY TIMER (5 min, with 30s warning toast) ---
        window.resetInactivityTimer = () => {
            ...
            // Show warning toast 30s before logout (after 4m30s of inactivity)
            window._inactivityWarningTimer = setTimeout(() => {
                if (!auth.currentUser) return;
                if (toast) toast.classList.remove('hidden');
                lucide.createIcons();
                let secs = 30;
                if (countdownEl) countdownEl.innerText = secs;
                window._inactivityCountdownInterval = setInterval(() => {
                    secs--;
                    if (countdownEl) countdownEl.innerText = secs;
                    if (secs <= 0) clearInterval(window._inactivityCountdownInterval);
                }, 1000);
            }, 4.5 * 60 * 1000); // 4 min 30s

            // Auto logout after 5 full minutes of inactivity
            window._inactivityLogoutTimer = setTimeout(() => {
                window.logoutUser(true);
            }, 5 * 60 * 1000); // 5 minutes
        };

        const _activityEvents = ['mousemove', 'keydown', 'click', 'scroll', 'touchstart'];
        _activityEvents.forEach(evt => document.addEventListener(evt, window.resetInactivityTimer, { passive: true }));
        ...
        function loadInventoryCategories() {
            ...
            if (select) {
                ...
                if (currentCat && currentCat !== 'new') select.value = currentCat;
            }
            ...
            const selectU = document.getElementById('inp-inv-unit');
            if (selectU) {
                const currentUnit = selectU.value;
                selectU.innerHTML = '';
                Array.from(units).sort().forEach(u => {
                    selectU.innerHTML += `<option value="${u}">${u}</option>`;
                });
                selectU.innerHTML += `<option value="new">+ Nuova Unità...</option>`;
                if (currentUnit && currentUnit !== 'new') selectU.value = currentUnit;
            }
        }
        ...
        window.handleScannedCode = async function (code) {
            ...
            // --- BATCH MODE ---
            if (isBatchMode) {
                if (userRole === 'user' && window.currentUserPermissions && !window.currentUserPermissions.inventory) {
                     alert("Accesso Negato: Non hai i permessi per il Magazzino.");
                     return;
                }
            ...
            if (toolSnap.exists()) {
                if (userRole === 'user' && window.currentUserPermissions && !window.currentUserPermissions.instruments) {
                    alert("Accesso Negato: Non hai i permessi per gli Strumenti. Rivolgiti a un amministratore.");
            ...
            if (invSnap.exists()) {
                if (userRole === 'user' && window.currentUserPermissions && !window.currentUserPermissions.inventory) {
                    alert("Accesso Negato: Non hai i permessi per il Magazzino. Rivolgiti a un amministratore.");
                    window.switchView('view-home');
                    return;
                }
        ...
        function showInventoryView(id, data) {
            ...
            // Notes
            const notesBlock = document.getElementById('inv-notes-block');
            const notesEl = document.getElementById('inv-item-notes');
            if (data.notes) {
        ...
        window.openStockModal = (action) => {
            try {
                ...
                const unit = (window.currentInventoryData && window.currentInventoryData.unit === 'pacco') ? 'pacchi' : (window.currentInventoryData ? window.currentInventoryData.unit : 'pz');

                document.getElementById('stock-modal-title').innerText = title;
                document.getElementById('stock-modal-unit').innerText = unit || 'pz';
                document.getElementById('inp-stock-qty').value = '';
                populateOperatorDropdown(); // Populate with PI + students

                // Gestione Checkbox rimuovi ordine
                if (action === 'add' && window.currentInventoryData && window.currentInventoryData.isOrdered) {
                ...
                alert("Errore in openStockModal: " + err.message);
                console.error(err);
            }
        };

        window.triggerOrderModal = () => {
            if (typeof currentItem !== 'undefined') {
                window.openOrderModal(currentItem);
            } else {
        ...
        window.showPublicInventoryList = () => {
            if (userRole === 'user' && window.currentUserPermissions && !window.currentUserPermissions.inventory) {
                return alert("Accesso Negato: Non hai l'abilitazione per visualizzare il Magazzino.");
            }
        ...
        window.loadPublicInventory = () => {
            ...
            onSnapshot(q, (snapshot) => {
                ...
                populateInventoryFilters(); // Fill public dropdowns
        ...
        function populateInventoryFilters() {
            ...
            const cats = new Set();
            const locs = new Set();
            window.allInventoryCache.forEach(i => {
                if (i.brand) brands.add(i.brand);
                if (i.category) cats.add(i.category);
                if (i.location) locs.add(i.location);
            });
            const renderOptions = (set, defaultText) => `<option value="">${defaultText}</option>` + 
                Array.from(set).sort().map(val => `<option value="${val}">${val}</option>`).join('');
                
            const brandEl = document.getElementById('filter-inv-brand');
            const catEl = document.getElementById('filter-inv-cat');
            const locEl = document.getElementById('filter-inv-loc');
            
            // Save current selection to restore after refresh
            const currBrand = brandEl?.value;
            const currCat = catEl?.value;
            const currLoc = locEl?.value;

            if(brandEl) { brandEl.innerHTML = renderOptions(brands, "Tutti i Marchi"); brandEl.value = currBrand || ''; }
            if(catEl) { catEl.innerHTML = renderOptions(cats, "Tutte le Categorie"); catEl.value = currCat || ''; }
            if(locEl) { locEl.innerHTML = renderOptions(locs, "Tutte le Posizioni"); locEl.value = currLoc || ''; }
        }

        window.filterInventoryList = () => {
            const searchInput = document.getElementById('inp-public-inv-search');
            ...
            const brandFilter = !isAdminView ? (document.getElementById('filter-inv-brand')?.value || '') : '';
            const catFilter = !isAdminView ? (document.getElementById('filter-inv-cat')?.value || '') : '';
            const locFilter = !isAdminView ? (document.getElementById('filter-inv-loc')?.value || '') : '';
            ...
            let filtered = window.allInventoryCache.filter(i => {
                ...
                return matchTerm && matchBrand && matchCat && matchLoc;
            });
        ...
        function renderInventoryList(items, container, isAdmin) {
            ...
            container.innerHTML = items.map(i => {
                ...
                const safeId = i.id.replace(/'/g, "\\'").replace(/"/g, "&quot;");
                const clickAction = isAdmin
                    ? `editInventoryItem('${safeId}')`
                    : `handleScannedCode('${safeId}')`;

                let quantityDisplay = `${i.quantity} <span class="text-[10px] text-slate-400">${i.unit}</span>`;
                if (i.size) {
                    quantityDisplay += ` <span class="text-[9px] text-slate-400 font-normal italic">(${i.size})</span>`;
                }
                if (i.unit === 'pacco') {
                ...
                return `
                <div onclick="${finalClickAction}" class="bg-white p-3 rounded-lg border ${isLow ? 'border-red-300 bg-red-50' : 'border-slate-100'} flex flex-col shadow-sm cursor-pointer hover:border-indigo-300">
                        ...
                        <div class="flex justify-between items-center w-full">
                            <div class="flex-1">
                                ...
                                <div class="text-[10px] text-slate-400 font-mono">${i.id} | ${i.location || '-'}</div>
                                 ${i.expiryDate ? `<div class="text-[10px] font-bold ${i.expiryDate < new Date().toISOString().slice(0,10) ? 'text-red-500' : 'text-slate-400'}">${i.expiryDate < new Date().toISOString().slice(0,10) ? '⚠ SCADUTO' : '📅'} Scad.: ${i.expiryDate}</div>` : ''}
                            </div>
        ...
        window.toggleBulkMode = () => {
            ...
            if (window.isBulkMode) {
                btnToggle.classList.add('bg-red-100', 'text-red-700', 'border-red-300');
                btnToggle.classList.remove('bg-slate-50', 'text-slate-600', 'border-slate-200');
                divActions.classList.remove('hidden');
            } else {
                btnToggle.classList.remove('bg-red-100', 'text-red-700', 'border-red-300');
                btnToggle.classList.add('bg-slate-50', 'text-slate-600', 'border-slate-200');
        ...
        window.toggleAllInventorySelection = (isChecked) => {
            document.querySelectorAll('.chk-inv-bulk').forEach(c => c.checked = isChecked);
            window.updateBulkDeleteUI();
        };

        window.bulkDeleteSelected = async () => {
            const ids = Array.from(document.querySelectorAll('.chk-inv-bulk:checked')).map(c => c.value);
            if (ids.length === 0) return;
            if (!confirm(`Sei sicuro di voler eliminare ${ids.length} articoli? Questa azione è irreversibile.`)) return;
            try {
                for (const id of ids) {
                    const safeId = id.replace(/\//g, '_');
                    await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'inventory', safeId));
                    logAction('DELETE', 'INVENTORY', `Bulk deleted: ${id}`);
                }
                alert(`${ids.length} articoli eliminati.`);
                window.isBulkMode = false;
                document.getElementById('btn-toggle-bulk')?.classList.remove('bg-red-100','text-red-700','border-red-300');
                document.getElementById('btn-toggle-bulk')?.classList.add('bg-slate-50','text-slate-600','border-slate-200');
                document.getElementById('div-bulk-actions')?.classList.add('hidden');
                filterInventoryList();
            } catch(err) {
        ...
        window.toggleNewUnit = () => {
            const select = document.getElementById('inp-inv-unit');
            const input = document.getElementById('inp-inv-unit-new');
            if (select.value === 'new') {
                input.classList.remove('hidden');
                input.required = true;
            } else {
        ...
        window.saveInventoryItem = async (e) => {
            ...
            let unit = document.getElementById('inp-inv-unit').value;
            if (unit === 'new') unit = document.getElementById('inp-inv-unit-new').value.trim();
            ...
            const size = document.getElementById('inp-inv-size').value.trim();
            const expiryDate = document.getElementById('inp-inv-expiry').value || '';
            ...
            try {
                ...
                const notes = document.getElementById('inp-inv-notes').value.trim();
                await setDoc(docRef, {
                    id, name, brand, unit, threshold, location: loc, size, expiryDate, quantity: finalQty, packSize: packSize, category: category || '', image: image || '', restockEmail: restockEmail || '', notes: notes || ''
                });
        ...
        window.clearInventoryForm = () => {
            ...
            document.getElementById('inp-inv-unit-new').classList.add('hidden');
            document.getElementById('inp-inv-threshold').value = '';
            document.getElementById('inp-inv-loc').value = '';
            document.getElementById('inp-inv-size').value = '';
            document.getElementById('inp-inv-expiry').value = '';
            ...
            document.getElementById('inp-inv-notes').value = '';
        ...
        window.editInventoryItem = (id) => {
            const item = (window.allInventoryCache || []).find(i => i.id === id);
            if (!item) return alert("Errore: Articolo non presente in cache.");

            const { name, unit, threshold, location, packSize, brand, category, image, quantity, restockEmail } = item;
            ...
            document.getElementById('inp-inv-threshold').value = threshold || 0;
            document.getElementById('inp-inv-loc').value = location || '';
            document.getElementById('inp-inv-size').value = item.size || '';
            document.getElementById('inp-inv-expiry').value = item.expiryDate || '';
            document.getElementById('inp-inv-image').value = image || '';
            document.getElementById('inp-inv-qty').value = (quantity !== undefined && quantity !== null) ? quantity : 0;
            document.getElementById('inp-inv-restock-email').value = restockEmail || '';
            document.getElementById('inp-inv-notes').value = item.notes || '';
            
            // Handle Unit select
            const unitSelect = document.getElementById('inp-inv-unit');
            let unitFound = false;
            ...
            if (!unitFound && unit) {
                unitSelect.value = 'new';
                document.getElementById('inp-inv-unit-new').classList.remove('hidden');
                document.getElementById('inp-inv-unit-new').value = unit;
            } else {
                document.getElementById('inp-inv-unit-new').classList.add('hidden');
            }
        ...
        window.downloadInventoryTemplate = () => {
            ...
            const headers = [["ID", "Nome", "Marchio", "Categoria", "Unita", "Capacità_Formato", "Data_Scadenza", "Quantita", "Pezzi_Pacco", "Soglia", "Posizione", "Immagine", "Email_Riordino", "Note"]];
            const example = [["ES-001", "Etanolo 96%", "Sigma-Aldrich", "Reagenti", "pz", "500 ml", "31-12-2026", 10, 1, 2, "Armadio A", "", "", ""]];
        ...
        // Date normalizer: accepts dd-mm-yyyy, dd/mm/yyyy, dd.mm.yyyy → yyyy-mm-dd
        function normalizeExpiryDate(val) {
            if (!val) return '';
            const s = val.toString().trim();
            if (/^\d{4}[-/]\d{2}[-/]\d{2}$/.test(s)) return s.replace(/\//g, '-');
            const m = s.match(/^(\d{1,2})[-/.]( \d{1,2})[-/.]( \d{4})$/) || s.match(/^(\d{1,2})[-/.](\d{1,2})[-/.](\d{4})$/);
            if (m) return `${m[3].trim()}-${m[2].trim().padStart(2,'0')}-${m[1].trim().padStart(2,'0')}`;
            return s;
        }
            ...
            if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
                reader.onload = async (e) => {
                    try {
                        ...
                        if (jsonData.length === 0) return alert("Il file Excel è vuoto.");
                        ...
                        for (const row of jsonData) {
                            ...
                            const safeId = id.replace(/\//g, '_');
                            await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'inventory', safeId), {
                                id,
                                name,
                                brand: (row["Marchio"] || "").toString().trim(),
                                category: (row["Categoria"] || "").toString().trim(),
                                unit: (row["Unita"] || "pz").toString().trim(),
                                size: (row["Capacità_Formato"] || "").toString().trim(),
                                expiryDate: normalizeExpiryDate(row["Data_Scadenza"]),
                                notes: (row["Note"] || "").toString().trim(),
                                quantity: parseFloat(row["Quantita"]) || 0,
                                packSize: parseFloat(row["Pezzi_Pacco"]) || 1,
                                threshold: parseFloat(row["Soglia"]) || 0,
                                location: (row["Posizione"] || "").toString().trim(),
                                image: (row["Immagine"] || "").toString().trim(),
                                restockEmail: (row["Email_Riordino"] || "").toString().trim()
                            }, { merge: true });
                            count++;
                        }
                        alert(`Importazione completata: ${count} articoli caricati da Excel.`);
                        logAction('IMPORT', 'INVENTORY', `Imported ${count} items from Excel.`);
                        input.value = '';
                    } catch (err) { alert("Errore durante l'importazione Excel: " + err); }
                };
                ...
                reader.onload = async (e) => {
                    ...
                    if (rows.length < 2) {
                        alert("Il file è vuoto o non contiene dati sufficienti.");
                        input.value = '';
                        return;
                    }
                    ...
                    delimiters.forEach(d => {
                        const c = countChar(headerRow, d);
                        if (c > maxCount) { maxCount = c; bestDelimiter = d; }
                    });
                    ...
                    for (let i = 1; i < rows.length; i++) {
                        ...
                        if (!line) continue;
                        const cols = line.split(bestDelimiter).map(p => p.trim().replace(/^"|"$/g, '').replace(/""/g, '"'));
                        if (cols.length < 2) continue;

                        // New order: ID, Name, Brand, Category, Unit, Size, ExpiryDate, Qty, PackSize, Threshold, Loc, Image, RestockEmail, Note
                        const [id, name, brand, category, unit, size, expiryDate, qty, packSize, threshold, loc, image, restockEmail, noteVal] = cols;
                        if (id && name) {
                            try {
                                await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'inventory', id.replace(/\//g, '_')), {
                                    id, name, 
                                    brand: brand || '', 
                                    category: category || '', 
                                    unit: unit || 'pz',
                    ...
                    const formatDelim = bestDelimiter === '\t' ? 'TAB' : bestDelimiter;
                    alert(`Importati/Aggiornati ${count} articoli utilizzando separatore '${formatDelim}'.`);
                    input.value = '';
                };
        ...
        window.switchAdminTab = (tab) => {
            ...
            ['bookings', 'instruments', 'labels', 'settings', 'access', 'inventory', 'logs', 'ai'].forEach(t => {
            ...
            if (tab === 'settings') { loadPiMapping().then(() => renderPiMappingUI()); }
            if (tab === 'ai') loadGeminiKey();
        };
        ...
        window.switchView = (id) => {
            ['view-scanner', 'view-details', 'view-admin', 'view-my-booking', 'view-inventory', 'view-inventory-list', 'view-batch-summary', 'view-statistiche'].forEach(v => {
        ...
        window.startScanner = async () => {
            try {
                // Stop any existing session first to prevent "camera in use" errors
                ...
                if (err && (err.name === 'NotAllowedError' || String(err).includes('Permission'))) {
                    alert('Accesso alla fotocamera negato. Controlla i permessi del browser.');
                } else {
                    alert('Impossibile avviare la fotocamera. Riprova o usa il campo manuale.');
                }
            }
        };
        window.stopScanner = async () => {
            try { if (html5QrCode && html5QrCode.isScanning) await html5QrCode.stop(); } catch(e) { console.warn('Stop scanner error:', e); }
            html5QrCode = null;
            resetApp();
        };
        ...
        async function showDetails(text) {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    currentItemName = data.description || text;
        ...
        window.currentBookingsView = 'active';

        window.switchBookingsView = (mode) => {
            window.currentBookingsView = mode;
            const btnActive = document.getElementById('btn-bookings-active');
            const btnArchived = document.getElementById('btn-bookings-archived');
            if (mode === 'active') {
                btnActive.className = 'flex-1 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold transition shadow-sm flex items-center justify-center gap-1.5';
                btnArchived.className = 'flex-1 py-2 bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold transition shadow-sm flex items-center justify-center gap-1.5';
            } else {
                btnArchived.className = 'flex-1 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold transition shadow-sm flex items-center justify-center gap-1.5';
                btnActive.className = 'flex-1 py-2 bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold transition shadow-sm flex items-center justify-center gap-1.5';
            }
            renderAdminBookings();
            if (window.lucide) lucide.createIcons();
        };
        ...
        window.renderAdminBookings = () => {
            ...
            const now = new Date();
            const mode = window.currentBookingsView || 'active';

            let filtered;
            if (mode === 'active') {
                filtered = bookings.filter(b => b.status !== 'archived');
            ...
            if (filtered.length === 0) {
                document.getElementById('admin-list').innerHTML = `<div class="text-center py-8 bg-white rounded-xl border border-slate-100"><p class="text-sm text-slate-400">${mode === 'active' ? 'Nessuna prenotazione attiva.' : 'Nessuna prenotazione archiviata.'}</p></div>`;
                return;
            }

            document.getElementById('admin-list').innerHTML = filtered.map(b => {
                const start = new Date(b.startDate);
                const end = new Date(b.endDate);
                const isActive = now >= start && now <= end;
                const isPast = now > end;
                const isUpcoming = !isActive && !isPast && (start - now) <= 24 * 3600000;

                let statusBadge = '';
                let borderClass = 'border-slate-100';
                if (b.status === 'archived') {
                    statusBadge = '<span class="text-[9px] font-bold px-2 py-0.5 bg-slate-100 text-slate-400 rounded uppercase">Conclusa</span>';
                    borderClass = 'border-slate-100 opacity-60';
                } else if (isActive) {
                    statusBadge = '<span class="text-[9px] font-bold px-2 py-0.5 bg-emerald-500 text-white rounded uppercase animate-pulse">In Uso</span>';
                    borderClass = 'border-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.15)]';
                } else if (isUpcoming) {
                    statusBadge = '<span class="text-[9px] font-bold px-2 py-0.5 bg-amber-400 text-amber-900 rounded uppercase">Imminente</span>';
                    borderClass = 'border-amber-300';
                } else {
                    statusBadge = '<span class="text-[9px] font-bold px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded uppercase">Futura</span>';
                    borderClass = 'border-indigo-100';
                }

                return `<div class="bg-white p-3 rounded-xl border-2 ${borderClass} text-xs shadow-sm">
                    <div class="flex justify-between items-start mb-1">
                        <div class="flex-1 min-w-0">
                            <div class="font-black text-sm text-slate-800">${b.resourceName || b.resourceId}</div>
                            <div class="text-[10px] text-slate-500">PNR: <span class="font-mono font-bold">${b.pnr || '—'}</span> · ${b.userName}</div>
                        </div>
                        <div class="flex items-center gap-2 flex-shrink-0">
                            ${statusBadge}
                            ${b.status !== 'archived' ? `<button onclick="editBookingFromAdmin('${b.id}')" class="p-1 bg-amber-100 rounded hover:bg-amber-200 transition"><i data-lucide="pencil" class="w-3 h-3"></i></button>` : ''}
                            <button onclick="deleteBooking('${b.id}')" class="p-1 bg-red-100 rounded hover:bg-red-200 transition"><i data-lucide="trash-2" class="w-3 h-3"></i></button>
                        </div>
                    </div>
                    <div class="flex gap-2 mt-1.5">
                        <div class="flex-1 bg-slate-50 rounded-lg p-1.5 text-center border border-slate-100">
                            <div class="text-[9px] text-slate-400 font-bold">INIZIO</div>
                            <div class="font-bold text-slate-700">${start.toLocaleString('it-IT', {dateStyle:'short', timeStyle:'short'})}</div>
                        </div>
                        <div class="flex-1 bg-slate-50 rounded-lg p-1.5 text-center border border-slate-100">
                            <div class="text-[9px] text-slate-400 font-bold">FINE</div>
                            <div class="font-bold text-slate-700">${end.toLocaleString('it-IT', {dateStyle:'short', timeStyle:'short'})}</div>
                        </div>
                    </div>
                </div>`;
            }).join('');
                ...
                document.getElementById('instruments-list').innerHTML = items.map(item => {
                    ...
                    <div id="inst-card-${item.id.replace(/\./g, '-')}" class="inst-card bg-white p-3 rounded-lg border border-slate-100 flex justify-between items-center shadow-sm cursor-pointer hover:border-indigo-400 transition" onclick="editInstrument('${item.id}')">
                        ...
                        <div class="flex gap-2">
                            <button onclick="event.stopPropagation(); deleteInstrument('${item.id}')" class="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
        ...
        window.deleteInstrument = async (id) => {
            if (!confirm(`Sei sicuro di voler eliminare lo strumento ${id}? Questa operazione è irreversibile.`)) return;
            try {
                await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'resources', id.replace(/\//g, '_')));
                logAction('DELETE', 'INSTRUMENT', `Instrument deleted: ${id}`);
                clearInstrumentForm();
            } catch (e) {
        ...
        // --- TOTEM ---
        function getTotemDisplayName(b) {
            let name = b.resourceName || b.resourceId;
            if (window.allInstrumentsCache && b.resourceId) {
                const inst = window.allInstrumentsCache.find(i => i.id === b.resourceId);
                if (inst && inst.description && inst.description !== b.resourceId) {
                    name = `${inst.description} (${b.resourceId})`;
                }
            }
            return name;
        }

        window.totemBookingsList = [];

        function renderTotemLists() {
            if (!window.totemBookingsList) return;
            const now = new Date();
            const active = window.totemBookingsList.filter(b => { const s = new Date(b.startDate), e = new Date(b.endDate); return now >= s && now <= e; });
            const upcoming = window.totemBookingsList.filter(b => new Date(b.startDate) > now).sort((a, b) => new Date(a.startDate) - new Date(b.startDate));
            
            document.getElementById('totem-active-list').innerHTML = active.map(b => `<div class="bg-slate-800 p-3 mb-2 rounded border-l-4 border-emerald-500"><h3 class="font-bold text-white">${getTotemDisplayName(b)}</h3><p class="text-emerald-400">${b.userName}</p></div>`).join('') || '<p class="text-slate-500">Nessuna attività</p>';
            document.getElementById('totem-upcoming-list').innerHTML = upcoming.map(b => `<div class="bg-slate-800 p-3 mb-2 rounded"><div class="flex justify-between"><span class="text-white font-bold">${getTotemDisplayName(b)}</span><span class="text-slate-400 text-sm">${new Date(b.startDate).toLocaleDateString('it-IT', { day: '2-digit', month: 'short' })}</span></div><div class="flex justify-between mt-1"><span class="text-indigo-400 text-sm">${b.userName}</span><span class="text-emerald-400 text-sm">${new Date(b.startDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${new Date(b.endDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div></div>`).join('') || '<p class="text-slate-500">Nessuna prenotazione</p>';
        }
        ...
        function initTotemMode() {
            ...
            loadInstruments(); // Make sure the cache is available for getTotemDisplayName
        ...
        function loadTotemData() {
            ...
            onSnapshot(q, (snapshot) => {
                const bookings = [];
                snapshot.forEach(doc => bookings.push({ id: doc.id, ...doc.data() }));
                window.totemBookingsList = bookings;
                renderTotemLists();
        ...
        function updateTotemStockAlerts() {
            ...
            onSnapshot(q, (snapshot) => {
                ...
                snapshot.forEach(docSnap => {
                    ...
                    all.push(i);
                    if ((i.quantity || 0) <= (i.threshold || 0) && i.threshold > 0) {
                        alerts.push(i);
                    }
                });

                // Update live stats bar (feature 2)
                const totalEl = document.getElementById('totem-inv-total');
                const alertStatEl = document.getElementById('totem-inv-alert-stat');
                const alertCountEl = document.getElementById('totem-inv-alert-count');
                if (totalEl) totalEl.textContent = all.length;
                if (alertStatEl && alertCountEl) {
                    if (alerts.length > 0) {
                        alertStatEl.classList.remove('hidden');
                        alertCountEl.textContent = alerts.length;
                    } else {
                        alertStatEl.classList.add('hidden');
                    }
                }

                const btn = document.getElementById('totem-stock-btn');
                const badge = document.getElementById('totem-stock-badge');
                const grid = document.getElementById('totem-stock-grid');
                if (!grid || !btn) return;
                ...
                // Render clickable stock cards (features 1 & 4)
                grid.innerHTML = alerts.map(i => {
                    const sizeStr = i.size ? ` <span class="text-[10px] text-slate-500">(${i.size})</span>` : '';
                    const isLow = (i.quantity || 0) <= 0;
                    return `<div onclick="openTotemRestockModal('${i.id}')"
                        class="bg-slate-800 border border-amber-500/40 rounded-lg p-3 cursor-pointer hover:bg-slate-700 hover:border-amber-400 transition group">
                        <p class="font-bold text-white text-sm truncate">${i.name}${sizeStr}</p>
                        <p class="text-slate-400 text-xs truncate">${i.brand || ''}</p>
                        <p class="text-amber-400 text-xs mt-2 font-mono">
                            Rimasti: <span class="${isLow ? 'text-red-500' : 'text-red-400'} font-bold">${i.quantity || 0}</span> ${i.unit || ''}
                            <span class="text-slate-600"> / soglia: ${i.threshold}</span>
                        </p>
                        <p class="text-indigo-400 text-[10px] mt-1 opacity-0 group-hover:opacity-100 transition">Clicca per rifornire →</p>
                    </div>`;
                }).join('');
            });
        }

        window.toggleTotemStockPanel = () => {
            const main = document.getElementById('totem-main-panel');
            const stockPanel = document.getElementById('totem-stock-panel');
            const isShowingStock = !stockPanel.classList.contains('hidden');
            if (isShowingStock) {
        ...
        // --- TOTEM QUICK RESTOCK MODAL ---
        let totemRestockItemId = null;
        let totemRestockAction = 'add';

        window.openTotemRestockModal = (itemId) => {
            const item = (window.allInventoryCache || []).find(i => i.id === itemId);
            if (!item) return;

            totemRestockItemId = itemId;
            totemRestockAction = 'add';

            document.getElementById('totem-restock-name').textContent = item.name;
            document.getElementById('totem-restock-brand').textContent = item.brand || '';
            document.getElementById('totem-restock-current-qty').textContent = item.quantity || 0;
            document.getElementById('totem-restock-unit').textContent = item.unit === 'pacco' ? 'pacchi' : (item.unit || 'pz');
            document.getElementById('totem-restock-qty').value = '';

            // Default to "add" highlighted
            setTotemRestockAction('add');

            document.getElementById('totem-restock-modal').classList.remove('hidden');
            setTimeout(() => document.getElementById('totem-restock-qty').focus(), 100);
        };

        window.closeTotemRestockModal = () => {
            document.getElementById('totem-restock-modal').classList.add('hidden');
            totemRestockItemId = null;
        };

        window.setTotemRestockAction = (action) => {
            totemRestockAction = action;
            const addBtn = document.getElementById('btn-totem-add');
            const removeBtn = document.getElementById('btn-totem-remove');
            if (action === 'add') {
                addBtn.className = 'py-2 rounded-lg text-sm font-bold bg-emerald-600/50 border border-emerald-400 text-emerald-300 transition';
                removeBtn.className = 'py-2 rounded-lg text-sm font-bold bg-slate-700 border border-slate-600 text-slate-400 hover:bg-slate-600 transition';
            } else {
                removeBtn.className = 'py-2 rounded-lg text-sm font-bold bg-red-600/50 border border-red-400 text-red-300 transition';
                addBtn.className = 'py-2 rounded-lg text-sm font-bold bg-slate-700 border border-slate-600 text-slate-400 hover:bg-slate-600 transition';
            }
        };

        window.confirmTotemRestock = async () => {
            if (!totemRestockItemId || !currentUser) return;
            const qty = parseFloat(document.getElementById('totem-restock-qty').value);
            if (!qty || qty <= 0) return alert('Inserisci una quantità valida.');

            const safeId = totemRestockItemId.replace(/\//g, '_');
            const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'inventory', safeId);
            ...
            try {
                ...
                await runTransaction(db, async (transaction) => {
                    ...
                    if (!sfDoc.exists()) throw 'Articolo non trovato!';
                    const currentQty = parseFloat(sfDoc.data().quantity || 0);
                    let newQty = totemRestockAction === 'add' ? currentQty + qty : currentQty - qty;
                    if (newQty < 0) throw 'Quantità insufficiente!';
                    transaction.update(docRef, { quantity: newQty });

                    const movRef = doc(collection(db, 'artifacts', appId, 'public', 'data', 'stock_movements'));
                    transaction.set(movRef, {
                        itemId: totemRestockItemId,
                        itemName: sfDoc.data().name,
                        action: totemRestockAction,
                        amount: qty,
                        unit: sfDoc.data().unit,
                        user: currentUser.uid,
                        operatorName: currentUser.displayName || currentUser.email || 'Totem',
                        timestamp: serverTimestamp()
                    });
                    updatedData = { ...sfDoc.data(), quantity: totemRestockAction === 'add' ? sfDoc.data().quantity + qty : sfDoc.data().quantity - qty };
                });

                // Update allInventoryCache so UI reflects immediately
                const idx = (window.allInventoryCache || []).findIndex(i => i.id === totemRestockItemId);
                if (idx !== -1) {
                    const snap = await getDoc(docRef);
                    if (snap.exists()) window.allInventoryCache[idx] = snap.data();
                }

                closeTotemRestockModal();
                logAction('UPDATE', 'INVENTORY', `Totem restock: ${totemRestockAction === 'add' ? '+' : '-'}${qty} for item ${totemRestockItemId}`);

                // Show success toast
                const toast = document.createElement('div');
                toast.className = 'fixed top-6 left-1/2 -translate-x-1/2 bg-emerald-600 text-white px-6 py-3 rounded-xl shadow-2xl z-[400] text-sm font-bold';
                toast.textContent = `✓ ${totemRestockAction === 'add' ? 'Rifornimento' : 'Prelievo'} confermato!`;
                document.body.appendChild(toast);
        ...
        window.goToFullItemDetail = async () => {
            if (!totemRestockItemId) return;
            const itemId = totemRestockItemId;
            closeTotemRestockModal();
            exitTotemMode();
            // Short delay to let the totem close, then navigate
            setTimeout(async () => {
                const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'inventory', itemId.replace(/\//g, '_'));
                const snap = await getDoc(docRef);
                if (snap.exists()) showInventoryView(itemId, snap.data());
            }, 300);
        };
...
<\/script>
        ...
        window.showMyBookingSearch = () => {
            if (userRole === 'user' && window.currentUserPermissions && !window.currentUserPermissions.instruments) {
                return alert("Accesso Negato: Non hai l'abilitazione per visualizzare le Prenotazioni.");
            }
        ...
        async function loadAccessSettings() {
            try {
                ...
                const permRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'permissions');
                const permSnap = await getDoc(permRef);
                const permData = permSnap.exists() ? (permSnap.data().users || {}) : {};
                const permContainer = document.getElementById('access-permissions-list');
                const permKeys = Object.keys(permData);
                
                if (permKeys.length === 0) {
                    permContainer.innerHTML = '<p class="text-[10px] p-2 bg-amber-100/50 rounded border border-amber-200 text-amber-800">Nessun permesso speciale assegnato. Tutti gli utenti usano le regole di default (Solo Strumenti).</p>';
                } else {
                    permContainer.innerHTML = permKeys.map(email => {
                        const p = permData[email];
                        const badges = [];
                        if (p.instruments) badges.push('<span class="bg-indigo-100 text-indigo-700 px-1 py-0.5 mt-1 sm:mt-0 rounded">Strumenti</span>');
                        if (p.inventory) badges.push('<span class="bg-emerald-100 text-emerald-700 px-1 py-0.5 mt-1 sm:mt-0 rounded">Magazzino</span>');
                        if (badges.length === 0) badges.push('<span class="bg-slate-100 text-slate-700 px-1 py-0.5 mt-1 sm:mt-0 rounded">Nessuno</span>');
                        return `
                        <div class="flex justify-between items-center bg-white p-2 rounded border border-amber-200 mb-1">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2 text-[10px]">
                                <span class="font-bold text-amber-900">${email}</span>
                                <div class="flex gap-1">
                                    ${badges.join('')}
                                </div>
                            </div>
                            <button onclick="removeUserPermission('${email}')" class="text-red-500 text-xs font-bold px-2 py-1 hover:bg-red-50 rounded shrink-0">✕</button>
                        </div>
                    `}).join('');
                }

                // Load Access Codes
                const supervisorsRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'supervisors');
                const supervisorsSnap = await getDoc(supervisorsRef);
                const supervisorsList = supervisorsSnap.exists() ? (supervisorsSnap.data().list || []) : [];

                const codesRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'accessCodes');
                const codesSnap = await getDoc(codesRef);
                const codesData = codesSnap.exists() ? (codesSnap.data().mappings || {}) : {};

                const allEmails = new Set();
                supervisorsList.forEach(s => allEmails.add(s.email.toLowerCase()));
                permKeys.forEach(e => allEmails.add(e.toLowerCase()));

                const codesContainer = document.getElementById('access-codes-list');
                if (allEmails.size === 0) {
                    codesContainer.innerHTML = '<p class="text-[10px] text-slate-400">Nessun utente/supervisore configurato per cui generare codici.</p>';
                } else {
                    let html = '';
                    for (const email of allEmails) {
                        const code = codesData[email] || '---';
                        
                        let roleDesc = 'Utente Standard (Strumenti)';
                        const isSup = supervisorsList.find(s => s.email.toLowerCase() === email);
                        if (isSup) {
                            const perms = [];
                            if (isSup.permissions.bookings) perms.push('Prenotazioni');
                            if (isSup.permissions.inventory) perms.push('Magazzino');
                            if (isSup.permissions.instruments) perms.push('Strumenti');
                            if (isSup.permissions.logs) perms.push('Logs');
                            roleDesc = `Supervisore (${perms.join(', ') || 'nessun modulo'})`;
                        } else if (permData[email]) {
                            const p = permData[email];
                            const perms = [];
                            if (p.instruments) perms.push('Strumenti');
                            if (p.inventory) perms.push('Magazzino');
                            roleDesc = `Utente Standard (${perms.join(', ') || 'nessuno'})`;
                        }

                        html += `
                        <div class="flex justify-between items-center bg-white p-2 rounded border border-indigo-200 mb-1 text-[10px]">
                            <div>
                                <div class="font-bold text-indigo-900">${email}</div>
                                <div class="text-[9px] text-slate-500">${roleDesc}</div>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="font-mono bg-slate-100 px-2 py-1 rounded text-slate-800 font-bold text-xs">${code}</span>
                                <button onclick="regenerateCodeForUser('${email}')" class="text-indigo-650 hover:text-indigo-800 font-bold">Rigenera</button>
                            </div>
                        </div>
                        `;
                    }
        ...
        // --- ACCESS CODES MANAGEMENT HELPERS ---
        function generateRandom5DigitCode() {
            return Math.floor(10000 + Math.random() * 90000).toString();
        }

        window.regenerateCodeForUser = async (email) => {
            try {
                const codesRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'accessCodes');
                const codesSnap = await getDoc(codesRef);
                const mappings = codesSnap.exists() ? (codesSnap.data().mappings || {}) : {};
                
                let code;
                let attempts = 0;
                do {
                    code = generateRandom5DigitCode();
                    attempts++;
                } while (Object.values(mappings).includes(code) && attempts < 100);

                mappings[email] = code;
                await setDoc(codesRef, { mappings }, { merge: true });
                logAction('UPDATE', 'SETTINGS', `Generated access code for: ${email}`);
                loadAccessSettings();
                alert(`Nuovo codice per ${email}: ${code}`);
            } catch (e) { alert('Errore: ' + e.message); }
        };

        window.printAccessCodes = async () => {
            try {
                const supervisorsSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'supervisors'));
                const supervisorsList = supervisorsSnap.exists() ? (supervisorsSnap.data().list || []) : [];

                const permissionsSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'permissions'));
                const permissionsData = permissionsSnap.exists() ? (permissionsSnap.data().users || {}) : {};

                const codesSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'accessCodes'));
                const codesData = codesSnap.exists() ? (codesSnap.data().mappings || {}) : {};

                const allEmails = new Set();
                supervisorsList.forEach(s => allEmails.add(s.email.toLowerCase()));
                Object.keys(permissionsData).forEach(e => allEmails.add(e.toLowerCase()));

                if (allEmails.size === 0) return alert('Nessun codice da stampare.');
                ...
                for (const email of allEmails) {
                    ...
                    if (isSup) {
                        ...
                        if (isSup.permissions.instruments) perms.push('Strumenti');
                        if (isSup.permissions.logs) perms.push('Logs');
                        roleDesc = `Supervisore (${perms.join(', ')})`;
                    } else if (permissionsData[email]) {
                        const p = permissionsData[email];
                        const perms = [];
                        if (p.instruments) perms.push('Strumenti');
                        if (p.inventory) perms.push('Magazzino');
                        roleDesc = `Utente Standard (${perms.join(', ')})`;
                    }

                    rowsHtml += `
                    <tr>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: left;">${email}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: left; color: #555;">${roleDesc}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: center; font-family: monospace; font-size: 16px; font-weight: bold; background: #f5f5f5; border-radius: 4px;">${code}</td>
                    </tr>
                    `;
                }
                ...
                <html>
                <head>
                    <title>UniScan - Codici di Accesso Rapido</title>
                    <style>
                        ...
                        th { background: #333; color: #fff; padding: 12px 10px; text-align: left; }
                ...
                <body>
                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #333; padding-bottom: 10px; margin-bottom: 20px;">
                        <h1 style="margin: 0; border: none; padding: 0;">🔑 UNISCAN - Codici di Accesso Rapido</h1>
                        <button onclick="window.print()" style="padding: 8px 16px; background: #10b981; color: white; border: none; border-radius: 6px; font-weight: bold; cursor: pointer;">STAMPA / SALVA PDF</button>
                    </div>
                    <p style="font-size: 12px; color: #666; margin-bottom: 20px;">Elenco aggiornato dei codici di accesso a 5 cifre per gli utenti autorizzati.</p>
                    <table>
                        <thead>
                            <tr>
                                <th style="text-align: left;">Email Utente</th>
                                <th style="text-align: left;">Ruolo / Permessi</th>
                                <th style="text-align: center; width: 100px;">Codice</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${rowsHtml}
            ...
            } catch (e) { alert('Errore durante la generazione della stampa: ' + e.message); }
        };

        window.generateAllMissingCodes = async () => {
            try {
                const supervisorsSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'supervisors'));
                const supervisorsList = supervisorsSnap.exists() ? (supervisorsSnap.data().list || []) : [];
                ...
                if (allEmails.size === 0) return alert('Nessun utente configurato.');
                ...
                if (count > 0) {
                    await setDoc(codesRef, { mappings }, { merge: true });
                    logAction('UPDATE', 'SETTINGS', `Generated ${count} missing access codes.`);
                    loadAccessSettings();
                    alert(`Generati ${count} nuovi codici.`);
                } else {
                    alert('Tutti gli utenti hanno già un codice associato.');
                }
            } catch (e) { alert('Errore: ' + e.message); }
        };

        window.regenerateAllCodes = async () => {
            if (!confirm('Sei sicuro di voler rigenerare TUTTI i codici? I vecchi codici non funzioneranno più.')) return;
            try {
                const supervisorsSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'supervisors'));
                const supervisorsList = supervisorsSnap.exists() ? (supervisorsSnap.data().list || []) : [];
                ...
                const codesRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'accessCodes');
                await setDoc(codesRef, { mappings });
                logAction('UPDATE', 'SETTINGS', 'Regenerated all access codes.');
                loadAccessSettings();
                alert('Tutti i codici sono stati rigenerati.');
            } catch (e) { alert('Errore: ' + e.message); }
        };

        // --- CODE-BASED LOGIN LOGIC ---
        let activeCodeEmail = null;

        window.showCodeLogin = () => {
            document.getElementById('login-form-container').classList.add('hidden');
            document.getElementById('code-login-container').classList.remove('hidden');
            document.getElementById('inp-login-code').value = '';
            document.getElementById('code-confirm-section').classList.add('hidden');
            document.getElementById('login-error').classList.add('hidden');
            activeCodeEmail = null;
        };

        window.showStandardLogin = () => {
            document.getElementById('login-form-container').classList.remove('hidden');
            document.getElementById('code-login-container').classList.add('hidden');
            document.getElementById('login-error').classList.add('hidden');
            activeCodeEmail = null;
        };

        window.handleCodeInput = async (inputEl) => {
            const val = inputEl.value.replace(/\D/g, '').slice(0, 5);
            inputEl.value = val;
            const errorEl = document.getElementById('login-error');
            errorEl.classList.add('hidden');
            document.getElementById('code-confirm-section').classList.add('hidden');
            ...
            if (val.length === 5) {
                try {
                    ...
                    if (!codesSnap.exists()) {
                        errorEl.innerText = "Nessun codice configurato nel sistema.";
                        errorEl.classList.remove('hidden');
                        return;
                    }
                    const mappings = codesSnap.data().mappings || {};
                    // Normalize all keys to lowercase for lookup
                    const normalizedMappings = {};
                    for (const k in mappings) normalizedMappings[k.toLowerCase()] = mappings[k];

                    let matchedEmail = null;
                    for (const email in normalizedMappings) {
                        if (normalizedMappings[email] === val) {
                            matchedEmail = email;
                            break;
                        }
                    }
                    ...
                    if (matchedEmail) {
                        activeCodeEmail = matchedEmail;
                        const msgEl = document.getElementById('code-confirm-msg');
                        msgEl.innerHTML = `<span class="text-indigo-600 text-base">✓</span> Codice trovato!<br><span class="text-slate-700 font-normal">Vuoi accedere come:</span><br><span class="text-indigo-800 font-mono text-xs break-all">${matchedEmail}</span>`;
                        document.getElementById('code-confirm-section').classList.remove('hidden');
                    } else {
                        errorEl.innerText = "Codice non valido o non assegnato.";
                        errorEl.classList.remove('hidden');
                    }
                } catch (e) {
                    errorEl.innerText = "Errore durante la verifica: " + e.message;
                    errorEl.classList.remove('hidden');
                }
            }
        };

        window.confirmCodeLogin = async () => {
            if (!activeCodeEmail) return;
            const errorEl = document.getElementById('login-error');
            errorEl.classList.add('hidden');
            
            const email = activeCodeEmail;
            const mappedEmail = email.replace('@', '+code@');
            const password = "UniscanUser_" + email.replace(/[^a-zA-Z0-9]/g, '');

            try {
                try {
                    await signInWithEmailAndPassword(auth, mappedEmail, password);
                } catch (err) {
                    if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-login-credentials') {
                        // User does not exist yet — auto register
                        await createUserWithEmailAndPassword(auth, mappedEmail, password);
                    } else {
                        throw err;
                    }
                }
            } catch (e) {
                errorEl.innerText = "Errore durante l'accesso: " + e.message;
                errorEl.classList.remove('hidden');
            }
        };
        ...
        window.addUserPermission = async () => {
            const emailInput = document.getElementById('inp-new-perm-email');
            const newEmail = emailInput.value.trim().toLowerCase();
            if (!newEmail || !newEmail.includes('@')) return alert('Email non valida.');
            
            const inst = document.getElementById('chk-perm-inst').checked;
            const inv = document.getElementById('chk-perm-inv').checked;

            try {
                const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'permissions');
                const snap = await getDoc(docRef);
                const data = snap.exists() ? snap.data() : { users: {} };
                const users = data.users || {};
                
                users[newEmail] = { instruments: inst, inventory: inv };
                
                await setDoc(docRef, { users }, { merge: true });
                emailInput.value = '';
                document.getElementById('chk-perm-inst').checked = true;
                document.getElementById('chk-perm-inv').checked = false;
                
                loadAccessSettings();
                logAction('UPDATE', 'SETTINGS', `User module permissions updated for: ${newEmail}`);
                alert(`Permessi aggiornati per ${newEmail}.`);
            } catch (e) { alert('Errore: ' + e.message); }
        };

        window.removeUserPermission = async (emailToRemove) => {
            if (!confirm(`Rimuovere le eccezioni per ${emailToRemove} (tornerà al default)?`)) return;
            try {
                const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'permissions');
                const snap = await getDoc(docRef);
                const data = snap.exists() ? snap.data() : { users: {} };
                ...
                if (users[emailToRemove]) {
                    ...
                    await setDoc(docRef, { users }); // update the full object
                    loadAccessSettings();
                    logAction('UPDATE', 'SETTINGS', `User module permissions removed for: ${emailToRemove}`);
                }
            } catch (e) { alert('Errore: ' + e.message); }
        };
        ...
        window.populateLogFilters = (logs) => {
            const catSelect = document.getElementById('log-filter-category');
            const userSelect = document.getElementById('log-filter-user');
            if (!catSelect || !userSelect) return;
            ...
            // Categories
            const cats = [...new Set(logs.map(l => l.category || 'SYSTEM'))].sort();
            catSelect.innerHTML = '<option value="">Tutte le Categorie</option>' + cats.map(c => `<option value="${c}">${c}</option>`).join('');
            catSelect.value = prevCat;

            // User emails
            const emails = [...new Set(logs.map(l => l.userEmail || l.userId).filter(Boolean))].sort();
            userSelect.innerHTML = '<option value="">Tutti gli Utenti (email)</option>' + emails.map(e => `<option value="${e}">${e}</option>`).join('');
            userSelect.value = prevUser;
        };

        window.applyLogFilters = () => {
            const logs = window.allLogsCache || [];
            const listEl = document.getElementById('admin-logs-list');
            if (!listEl) return;

            const filterCat = (document.getElementById('log-filter-category')?.value || '').trim();
            const filterUser = (document.getElementById('log-filter-user')?.value || '').trim();
            const filterOp = (document.getElementById('log-filter-operator')?.value || '').trim().toLowerCase();
            const filterSearch = (document.getElementById('log-filter-search')?.value || '').trim().toLowerCase();

            let filtered = logs;
            if (filterCat) filtered = filtered.filter(l => (l.category || 'SYSTEM') === filterCat);
            if (filterUser) filtered = filtered.filter(l => (l.userEmail || l.userId || '') === filterUser);
            if (filterOp) filtered = filtered.filter(l => (l.details || '').toLowerCase().includes(filterOp));
            if (filterSearch) {
                filtered = filtered.filter(l => {
                    const hay = `${l.action || ''} ${l.details || ''} ${l.userEmail || ''} ${l.category || ''}`.toLowerCase();
            ...
            if (countEl) countEl.textContent = `${filtered.length} di ${logs.length} log`;

            if (filtered.length === 0) {
                listEl.innerHTML = '<p class="text-center text-xs text-slate-400">Nessun log trovato con i filtri selezionati.</p>';
                return;
            }

            listEl.innerHTML = filtered.slice(0, 200).map(l => {
                ...
                if (l.category === 'EMAIL') color = 'bg-rose-50 text-rose-600';
                if (l.category === 'ERROR') color = 'bg-red-50 text-red-600';
                ...
                return `
                     <div class="p-2 border-l-4 border-slate-300 bg-white shadow-sm mb-1 text-[10px] md:text-xs">
                         <div class="flex justify-between items-center mb-1">
                             <span class="font-bold px-1.5 py-0.5 rounded ${color} text-[9px] uppercase tracking-wider">${l.category || 'SYSTEM'}</span>
        ...
        window.clearLogFilters = () => {
            ...
            const opEl = document.getElementById('log-filter-operator');
        ...
        window.exportInventoryCSV = () => {
            ...
            const data = window.allInventoryCache.map(i => ({
                "ID": i.id,
                "Nome": i.name,
                "Marchio": i.brand || '',
                "Categoria": i.category || '',
                "Unita": i.unit,
                "Capacità_Formato": i.size || '',
                "Data_Scadenza": i.expiryDate || '',
                "Quantita": i.quantity || 0,
                "Pezzi_Pacco": i.packSize || 1,
                "Soglia": i.threshold || 0,
                "Posizione": i.location || '',
                "Immagine": i.image || '',
                "Email_Riordino": i.restockEmail || '',
                "Note": i.notes || ''
            }));
        ...
        window.showStatistiche = async () => {
            if (userRole === 'user') return alert('Accesso negato.');
            switchView('view-statistiche');
            window.piMappingCache = null; // Force reload
            await loadPiMapping();
            populateAnalyticsFilters();
            loadAnalyticsData();
        };

        window.currentAnalyticsRange = '30d';
        window.analyticsCharts = {};

        window.changeAnalyticsRange = (range) => {
            window.currentAnalyticsRange = range;
            // Update button styles
            document.querySelectorAll('.stat-range-btn').forEach(btn => {
                btn.className = 'stat-range-btn px-3 py-1.5 rounded-lg text-[10px] font-bold border transition bg-white text-slate-600 border-slate-200 hover:bg-slate-50';
            });
            const activeBtn = document.getElementById('stat-range-' + range);
            if (activeBtn) activeBtn.className = 'stat-range-btn px-3 py-1.5 rounded-lg text-[10px] font-bold border transition bg-violet-600 text-white border-violet-600';
            loadAnalyticsData();
        };
        ...
        window.loadAnalyticsData = async () => {
            ...
            const chartsEl = document.getElementById('analytics-charts-container');
            if (loadingEl) loadingEl.classList.remove('hidden');
            if (chartsEl) chartsEl.classList.add('opacity-50');

            try {
                const now = new Date();
                let cutoff = null;
                const range = window.currentAnalyticsRange || '30d';
                if (range === '7d')  cutoff = new Date(now - 7 * 86400000);
                if (range === '30d') cutoff = new Date(now - 30 * 86400000);
                if (range === '3m')  cutoff = new Date(now - 90 * 86400000);

                // Fetch all stock_movements
                const movSnap = await getDocs(query(collection(db, 'artifacts', appId, 'public', 'data', 'stock_movements'), orderBy('timestamp', 'desc')));
                let movements = [];
                movSnap.forEach(d => movements.push({...d.data(), _id: d.id}));
                
                // Get all bookings for filtering and stats
                const bookSnap = await getDocs(query(collection(db, 'artifacts', appId, 'public', 'data', 'bookings')));
                let bookings = [];
                bookSnap.forEach(d => bookings.push({id: d.id, ...d.data()}));

                // Load PI mapping for filters
                await loadPiMapping();

                const filterPI = document.getElementById('stat-filter-pi').value;
                const filterUser = document.getElementById('stat-filter-user').value;

                // Apply date cutoff
                if (cutoff) {
                    movements = movements.filter(m => m.timestamp?.seconds && new Date(m.timestamp.seconds * 1000) >= cutoff);
                    bookings = bookings.filter(b => new Date(b.startDate) >= cutoff);
                }

                // Apply PI/User Filters
                if (filterUser) {
                    // Filter specific user
                    movements = movements.filter(m => normalizeUserName(m.operatorName || m.user) === filterUser);
                    bookings = bookings.filter(b => normalizeUserName(b.email || b.userName || b.bookedBy) === filterUser);
                } else if (filterPI) {
                    // Filter by PI: include PI + all their students
                    const piData = (window.piMappingCache || []).find(p => p.name === filterPI);
                    const allowedNames = new Set();
                    allowedNames.add(filterPI);
                    if (piData && piData.students) piData.students.forEach(s => allowedNames.add(s));
                    movements = movements.filter(m => allowedNames.has(normalizeUserName(m.operatorName || m.user)));
                    bookings = bookings.filter(b => allowedNames.has(normalizeUserName(b.email || b.userName || b.bookedBy)));
                }

                // Get inventory for health & KPIs
                const invArr = window.allInventoryCache || [];

                // Update KPIs
                updateAnalyticsKPIs(invArr, bookings, movements);

                // Build charts
                buildAnalyticsCharts(movements, bookings, invArr);
            } catch(e) { console.error('Analytics load error:', e); }
            finally {
                if (loadingEl) loadingEl.classList.add('hidden');
                if (chartsEl) chartsEl.classList.remove('opacity-50');
            }
        };

        window.updateAnalyticsKPIs = (inventory, bookings, movements) => {
            const now = new Date();
            const lowStock = inventory.filter(i => i.quantity <= (i.threshold || 0)).length;
            const activeNow = bookings.filter(b => {
                const s = new Date(b.startDate), e = new Date(b.endDate);
                return now >= s && now <= e;
            }).length;
            const bookingsPeriod = bookings.length;

            const instrCounts = {};
            bookings.forEach(b => {
                const n = b.resourceName || b.resourceId || '?';
                instrCounts[n] = (instrCounts[n] || 0) + 1;
            });
            const topInstr = Object.entries(instrCounts).sort((a,b) => b[1]-a[1])[0];

            const el = (id) => document.getElementById(id);
            if (el('kpi-total-items')) el('kpi-total-items').textContent = inventory.length;
            if (el('kpi-low-stock')) el('kpi-low-stock').textContent = lowStock;
            if (el('kpi-active-bookings')) el('kpi-active-bookings').textContent = activeNow;
            if (el('kpi-bookings-month')) el('kpi-bookings-month').textContent = bookingsPeriod;
            if (el('kpi-top-instrument')) el('kpi-top-instrument').textContent = topInstr ? topInstr[0] : '\u2014';
            if (el('kpi-top-instrument-count')) el('kpi-top-instrument-count').textContent = topInstr ? `${topInstr[1]} prenotazioni nel periodo` : '';
        };

        window.buildAnalyticsCharts = (movements, bookings, inventory) => {
            // Destroy existing
            Object.values(window.analyticsCharts || {}).forEach(c => { try { c.destroy(); } catch(e) {} });
            window.analyticsCharts = {};
            const bgColors = ['#10b981','#3b82f6','#f59e0b','#8b5cf6','#ec4899','#64748b','#0ea5e9','#f43f5e','#84cc16','#a855f7','#14b8a6','#f97316'];

            // 1. Consumption over time (line)
            const consumCtx = document.getElementById('chart-consumption');
            if (consumCtx) {
                const removals = movements.filter(m => m.action === 'remove');
                const weekBuckets = {};
                removals.forEach(m => {
                    if (!m.timestamp?.seconds) return;
                    const d = new Date(m.timestamp.seconds * 1000);
                    const ws = new Date(d); ws.setDate(d.getDate() - d.getDay());
                    const key = ws.toLocaleDateString('it-IT', {day:'2-digit', month:'short'});
                    weekBuckets[key] = (weekBuckets[key] || 0) + 1; // Count operations instead of mixing units
                });
                window.analyticsCharts.consumption = new Chart(consumCtx, {
                    type: 'line',
                    data: { labels: Object.keys(weekBuckets), datasets: [{ label: 'Prelievi', data: Object.values(weekBuckets), borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: '#ef4444' }] },
                    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
                });
            }

            // 2. Top consumed materials (horizontal bar)
            const topCtx = document.getElementById('chart-top-consumed');
            if (topCtx) {
                const itemCounts = {};
                movements.filter(m => m.action === 'remove').forEach(m => {
                    const name = m.itemName || m.itemId || '?';
                    itemCounts[name] = (itemCounts[name] || 0) + 1; // Count operations instead of raw quantity
                });
                const sorted = Object.entries(itemCounts).sort((a,b) => b[1]-a[1]).slice(0, 10);
                window.analyticsCharts.topConsumed = new Chart(topCtx, {
                    type: 'bar',
                    data: { labels: sorted.map(s=>s[0]), datasets: [{ label:'N° Prelievi', data: sorted.map(s=>s[1]), backgroundColor: bgColors, borderRadius: 6 }] },
            ...
            // 3. Instrument usage (bar)
            const instrCtx = document.getElementById('chart-instrument-usage');
            if (instrCtx) {
                const instrCounts = {};
                bookings.forEach(b => {
                    const name = b.resourceName || b.resourceId || '?';
                    instrCounts[name] = (instrCounts[name] || 0) + 1;
                });
                const sorted = Object.entries(instrCounts).sort((a,b) => b[1]-a[1]).slice(0, 10);
                window.analyticsCharts.instrUsage = new Chart(instrCtx, {
                    type: 'bar',
                    data: { labels: sorted.map(s=>s[0]), datasets: [{ label:'Prenotazioni', data: sorted.map(s=>s[1]), backgroundColor: '#6366f1', borderRadius: 6 }] },
                    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
                });
            }

            // 4. Bookings per day of week
            const heatCtx = document.getElementById('chart-booking-heatmap');
            if (heatCtx) {
                const dayNames = ['Dom','Lun','Mar','Mer','Gio','Ven','Sab'];
                const dayCounts = [0,0,0,0,0,0,0];
                bookings.forEach(b => { dayCounts[new Date(b.startDate).getDay()]++; });
                window.analyticsCharts.heatmap = new Chart(heatCtx, {
                    type: 'bar',
                    data: { labels: dayNames, datasets: [{ label:'Prenotazioni', data: dayCounts, backgroundColor: dayCounts.map((v,i) => i===0||i===6 ? '#94a3b8' : '#8b5cf6'), borderRadius: 6 }] },
                    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
                });
            }

            // 5. Inventory health (doughnut)
            const healthCtx = document.getElementById('chart-inventory-health');
            if (healthCtx) {
                let healthy = 0, low = 0, out = 0;
                inventory.forEach(i => {
                    if (i.quantity <= 0) out++;
                    else if (i.quantity <= (i.threshold || 0)) low++;
                    else healthy++;
                });
                window.analyticsCharts.health = new Chart(healthCtx, {
                    type: 'doughnut',
                    data: { labels: ['Disponibile','Scorta bassa','Esaurito'], datasets: [{ data: [healthy, low, out], backgroundColor: ['#10b981','#f59e0b','#ef4444'], borderWidth: 0 }] },
                    options: { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'bottom', labels: { font: { size: 11, weight: 'bold' }, padding: 12 } } } }
                });
            }

            // 6. Category breakdown by total quantity (pie)
            const catCtx = document.getElementById('chart-category-breakdown');
            if (catCtx) {
                const catQty = {};
                inventory.forEach(i => {
                    const c = i.category || 'Altro';
                    catQty[c] = (catQty[c] || 0) + (i.quantity || 0);
                });
                window.analyticsCharts.catBreakdown = new Chart(catCtx, {
                    type: 'pie',
                    data: { labels: Object.keys(catQty), datasets: [{ data: Object.values(catQty), backgroundColor: bgColors, borderWidth: 1, borderColor: '#fff' }] },
                    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { font: { size: 10, weight: 'bold' }, padding: 8 } } } }
                });
            }
        ...
        // --- BOOKING ARCHIVAL ---
        window.archiveExpiredBookings = async () => {
            try {
                const bookSnap = await getDocs(query(collection(db, 'artifacts', appId, 'public', 'data', 'bookings')));
                const now = new Date();
                const batch = [];
                bookSnap.forEach(d => {
                    const b = d.data();
                    if (new Date(b.endDate) < now && b.status !== 'archived') {
                        batch.push(d.id);
                    }
                });
                ...
                if (batch.length > 0) console.log(`Archived ${batch.length} expired bookings.`);
            } catch(e) { console.warn('Archive bookings failed:', e); }
        };

        // --- OPERATOR DROPDOWN FOR STOCK MOVEMENTS ---
        window.populateOperatorDropdown = async () => {
            await loadPiMapping();
            const sel = document.getElementById('inp-stock-user-id');
            if (!sel) return;
            sel.innerHTML = '<option value="" disabled selected>Seleziona operatore...</option>';
            const pis = window.piMappingCache || [];
            pis.forEach(pi => {
                // PI as optgroup with their students
                const group = document.createElement('optgroup');
                group.label = `\ud83d\udc54 ${pi.name} (PI)`;
                // Add PI themselves as an option
                const piOpt = document.createElement('option');
                piOpt.value = pi.name;
                piOpt.textContent = pi.name;
                group.appendChild(piOpt);
                // Add students
                (pi.students || []).sort().forEach(s => {
        ...
        // --- PI MAPPING: Firestore-backed name normalization ---
        window.piMappingCache = null;

        window.loadPiMapping = async () => {
            if (window.piMappingCache !== null) return; // already loaded
            try {
                const snap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'pi_mapping'));
                if (snap.exists()) {
                    window.piMappingCache = snap.data().pis || [];
                } else {
                    window.piMappingCache = [];
                }
            } catch(e) { console.warn('PI mapping load failed:', e); window.piMappingCache = []; }
        };

        window.normalizeUserName = (input) => {
            if (!input) return null;
            const str = input.toLowerCase().trim();
            
            // Build dynamic mapping from piMappingCache
            const mapping = {};
            (window.piMappingCache || []).forEach(pi => {
                // Add PI name parts as keys
                const piParts = pi.name.toLowerCase().split(' ');
                piParts.forEach(part => { if (part.length > 2) mapping[part] = pi.name; });
                mapping[pi.name.toLowerCase()] = pi.name;
                
                // Add student name parts as keys
                (pi.students || []).forEach(st => {
                    const stParts = st.toLowerCase().split(' ');
                    stParts.forEach(part => { if (part.length > 2) mapping[part] = st; });
                    mapping[st.toLowerCase()] = st;
                });
            });
            ...
            for (const [key, canonical] of Object.entries(mapping)) {
                if (str.includes(key) && key.length > 2) return canonical;
            }
            return input; // Fallback: return as-is
        };

        window.populateAnalyticsFilters = () => {
            const piSel = document.getElementById('stat-filter-pi');
            const userSel = document.getElementById('stat-filter-user');
            if (!piSel || !userSel) return;

            // Populate PI dropdown
            const currentPi = piSel.value;
            piSel.innerHTML = '<option value="">Tutti i PI</option>';
            (window.piMappingCache || []).forEach(pi => {
                const opt = document.createElement('option');
                opt.value = pi.name;
                opt.textContent = pi.name;
                if (pi.name === currentPi) opt.selected = true;
                piSel.appendChild(opt);
            });

            // Populate User dropdown based on PI selection
            const currentUser = userSel.value;
            userSel.innerHTML = '<option value="">Tutti gli Utilizzatori</option>';
            const selectedPI = piSel.value;

            if (selectedPI) {
                // Show only students of selected PI + PI themselves
                const piData = (window.piMappingCache || []).find(p => p.name === selectedPI);
                if (piData) {
                    // Add the PI themselves
                    const piOpt = document.createElement('option');
                    piOpt.value = piData.name;
                    piOpt.textContent = `👔 ${piData.name} (PI)`;
                    if (piData.name === currentUser) piOpt.selected = true;
                    userSel.appendChild(piOpt);
                    // Add students
                    (piData.students || []).sort().forEach(s => {
                        const opt = document.createElement('option');
                        opt.value = s;
                        opt.textContent = s;
                        if (s === currentUser) opt.selected = true;
                        userSel.appendChild(opt);
                    });
                }
            } else {
                // Show all known users from the mapping
                const allUsers = new Set();
                (window.piMappingCache || []).forEach(pi => {
                    allUsers.add(pi.name);
                    (pi.students || []).forEach(s => allUsers.add(s));
                });
                Array.from(allUsers).sort().forEach(u => {
                    const opt = document.createElement('option');
                    opt.value = u;
                    const isPi = (window.piMappingCache || []).some(p => p.name === u);
                    opt.textContent = isPi ? `👔 ${u}` : u;
        ...
        window.onPiFilterChange = () => {
            // Reset student filter when PI changes, then repopulate
            document.getElementById('stat-filter-user').value = '';
            populateAnalyticsFilters();
        };

        // --- PI MAPPING ADMIN CRUD ---
        window.savePiMapping = async () => {
            try {
                await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'pi_mapping'), {
                    pis: window.piMappingCache || [],
                    updatedAt: serverTimestamp()
                });
            } catch(e) { alert('Errore salvataggio PI mapping: ' + e); }
        };

        window.addNewPI = async () => {
            const name = document.getElementById('inp-new-pi-name').value.trim();
            if (!name) return alert('Inserisci un nome.');
            if (!window.piMappingCache) window.piMappingCache = [];
            if (window.piMappingCache.some(p => p.name.toLowerCase() === name.toLowerCase())) return alert('PI già presente.');
            window.piMappingCache.push({ name, students: [] });
            await savePiMapping();
            document.getElementById('inp-new-pi-name').value = '';
            renderPiMappingUI();
            logAction('UPDATE', 'SETTINGS', `Added PI: ${name}`);
        };

        window.removePI = async (piName) => {
            if (!confirm(`Rimuovere il PI "${piName}" e tutti i suoi utilizzatori assegnati?`)) return;
            window.piMappingCache = (window.piMappingCache || []).filter(p => p.name !== piName);
            await savePiMapping();
            renderPiMappingUI();
            logAction('UPDATE', 'SETTINGS', `Removed PI: ${piName}`);
        };

        window.addStudentToPI = async (piName) => {
            const inp = document.getElementById('inp-student-' + piName.replace(/\s/g, '_'));
            const studentName = inp ? inp.value.trim() : '';
            if (!studentName) return alert('Inserisci il nome dello studente.');
            const pi = (window.piMappingCache || []).find(p => p.name === piName);
            if (!pi) return;
            if (pi.students.some(s => s.toLowerCase() === studentName.toLowerCase())) return alert('Utilizzatore già presente.');
            pi.students.push(studentName);
            await savePiMapping();
            if (inp) inp.value = '';
            renderPiMappingUI();
            logAction('UPDATE', 'SETTINGS', `Added student ${studentName} to PI ${piName}`);
        };

        window.removeStudentFromPI = async (piName, studentName) => {
            const pi = (window.piMappingCache || []).find(p => p.name === piName);
            if (!pi) return;
            pi.students = pi.students.filter(s => s !== studentName);
            await savePiMapping();
            renderPiMappingUI();
        };

        window.renderPiMappingUI = () => {
            const container = document.getElementById('pi-mapping-ui');
            if (!container) return;
            const pis = window.piMappingCache || [];
            if (pis.length === 0) {
                container.innerHTML = '<p class="text-[10px] text-slate-400 text-center py-2">Nessun PI configurato. Aggiungine uno qui sotto.</p>';
                return;
            }
            container.innerHTML = pis.map(pi => {
                const safeId = pi.name.replace(/\s/g, '_');
                const studentsList = (pi.students || []).map(s => 
                    `<div class="flex items-center justify-between bg-white rounded px-2 py-1 border border-slate-100">
                        <span class="text-[10px] text-slate-700">${s}</span>
                        <button onclick="removeStudentFromPI('${pi.name}', '${s}')" class="text-red-400 hover:text-red-600 text-xs font-bold ml-2">&times;</button>
                    </div>`
                ).join('');
                return `<div class="bg-white rounded-lg border border-violet-100 p-3 shadow-sm">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-xs font-bold text-violet-800 flex items-center gap-1">👔 ${pi.name}</span>
                        <button onclick="removePI('${pi.name}')" class="text-[10px] text-red-500 font-bold hover:underline">Rimuovi PI</button>
                    </div>
                    <div class="space-y-1 mb-2">${studentsList || '<p class="text-[10px] text-slate-400 italic">Nessun utilizzatore assegnato</p>'}</div>
                    <div class="flex gap-1">
                        <input type="text" id="inp-student-${safeId}" class="flex-1 text-[10px] p-1.5 border border-slate-200 rounded" placeholder="Nome e Cognome Utilizzatore">
                        <button onclick="addStudentToPI('${pi.name}')" class="px-2 py-1 bg-violet-600 text-white rounded text-[10px] font-bold hover:bg-violet-700">+ Aggiungi</button>
                    </div>
                </div>`;
            }).join('');
        ...
        // --- GEMINI AI ASSISTANT ---
        window.loadGeminiKey = () => {
            const el = document.getElementById('inp-gemini-key');
            if (el) el.value = localStorage.getItem('gemini_api_key') || '';
        };

        window.saveGeminiKey = () => {
            const val = document.getElementById('inp-gemini-key').value.trim();
            if (val) {
                localStorage.setItem('gemini_api_key', val);
                alert("Chiave Gemini salvata in locale sul browser.");
            } else {
        ...
        window.generateAiReport = async () => {
            ...
            if (!apiKey) return alert("Inserisci prima la tua Gemini API Key.");
            ...
            try {
                // Leggere i filtri UI
                const timeDays = parseInt(document.getElementById('ai-filter-time').value || '7');
                const filterScope = document.getElementById('ai-filter-scope').value || 'all';

                const now = new Date();
                const cutoff = new Date(now - timeDays * 86400000);
                
                let movList = [];
                let totalBook = 0;

                // Estrazione DB condizionata
                if (filterScope === 'all' || filterScope === 'inventory') {
                    const movSnap = await getDocs(query(collection(db, 'artifacts', appId, 'public', 'data', 'stock_movements'), orderBy('timestamp', 'desc')));
                    movSnap.forEach(d => {
                        const m = d.data();
                        if (m.timestamp?.seconds && new Date(m.timestamp.seconds*1000) >= cutoff) movList.push(m);
                    });
                }

                if (filterScope === 'all' || filterScope === 'instruments') {
                    const bookSnap = await getDocs(query(collection(db, 'artifacts', appId, 'public', 'data', 'bookings')));
                    bookSnap.forEach(d => {
                        const m = d.data();
                        if (new Date(m.startDate) >= cutoff) {
                            totalBook++;
                        }
                    });
                }

                let promptText = `Sei l'assistente IA del Bioinformatics Lab. Scrivi un report professionale e conciso delle attività degli ultimi ${timeDays} giorni.
Usa elenchi puntati per facilitare la lettura. Il tono dev'essere istituzionale ma diretto. 

DATI DI RIEPILOGO:
`;
                if (filterScope === 'all' || filterScope === 'instruments') {
                    promptText += `- Nuove Prenotazioni Strumenti nel periodo: ${totalBook}\n`;
                }
                
                if (filterScope === 'all' || filterScope === 'inventory') {
                    promptText += `- Totale Movimenti Magazzino Registrati: ${movList.length}\n\n`;
                    promptText += `DETTAGLIO MOVIMENTI MAGAZZINO:\n`;
                    
                    const agg = {};
                    const userAgg = {};
                    movList.forEach(m => {
                        const name = m.itemName || 'Sconosciuto';
                        const user = normalizeUserName(m.operatorName || m.user) || 'Anonimo';
                        
                        if (!agg[name]) agg[name] = { add:0, sub:0 };
                        if (!userAgg[user]) userAgg[user] = { add:0, sub:0, items: new Set() };

                        if (m.action === 'add') {
                            agg[name].add += Number(m.amount || 0);
                            userAgg[user].add += Number(m.amount || 0);
                            userAgg[user].items.add(name);
                        }
                        if (m.action === 'remove' || m.action === 'sub') {
                            agg[name].sub += Number(m.amount || 0);
                            userAgg[user].sub += Number(m.amount || 0);
                            userAgg[user].items.add(name);
                        }
                    });
                    ...
                    for (const [name, qty] of Object.entries(agg)) {
                        hasMv = true;
                        promptText += `- ${name}: +${qty.add} aggiunti, -${qty.sub} prelevati\n`;
                    }
                    
                    promptText += `\nATTIVITÀ PER UTILIZZATORE:\n`;
                    for (const [u, acts] of Object.entries(userAgg)) {
                        if (acts.sub > 0 || acts.add > 0) {
                            const itemsArr = Array.from(acts.items).join(', ');
                            promptText += `- ${u}: prelevate ${acts.sub} unità, aggiunte ${acts.add} unità. (Articoli movimentati: ${itemsArr})\n`;
                        }
                    }

                    if (!hasMv) promptText += "Nessun movimento significativo registrato.\n";
                }

                promptText += `\nGenera il report in Markdown e formattalo in modo professionale (sii molto analitico e discorsivo, non restituirmi solo liste formattate).
Obbligatorio:
1. Inizia con un titolo appropriato ai filtri richiesti (es. # Report Activity Lab [Giorni/Ambito]).`;

                if (filterScope === 'all' || filterScope === 'inventory') {
                    promptText += ` 
2. Fai una classifica discorsiva: menziona esplicitamente gli operatori/utilizzatori che hanno fatto più prelievi indicando cosa hanno prelevato maggiormente.
3. Evidenzia quali reagenti o articoli sono stati maggiormente intaccati (se ce ne sono).`;
                }
                ...
                window.currentAiReportText = aiText;

                // 3. Render
                document.getElementById('ai-report-content').innerHTML = window.marked ? marked.parse(aiText) : aiText.replace(/\n/g, '<br>');
                document.getElementById('ai-report-container').classList.remove('hidden');

            } catch (err) {
                console.error(err);
                alert("Si è verificato un errore durante la generazione: " + err.message);
            } finally {
        ...
        window.downloadAiReportTXT = () => {
            if (!window.currentAiReportText) return;
            const blob = new Blob([window.currentAiReportText], { type: 'text/plain' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'AI_Lab_Report.txt';
            a.click();
            URL.revokeObjectURL(url);
        };
